...
export default function Complexe() {
  ...
  return (
    <Layout>
      ...
      {/* En-tête */}
      <section style={{ background: 'linear-gradient(105deg,#2a1457,#3d1e7b 55%,rgba(220,141,50,.85))', color: '#fff', padding: '56px 26px' }}>
      ...
      <section style={{ padding: '54px 26px 24px', maxWidth: 1240, margin: '0 auto' }}>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(300px,1fr))', gap: 36, alignItems: 'center' }}>
          <div>
            <h2 style={{ fontFamily: "'Oswald',sans-serif", fontSize: 'clamp(24px,3vw,34px)', fontWeight: 700, textTransform: 'uppercase', color: '#3d1e7b', margin: '0 0 14px' }}>Complexe Henri Barbusse</h2>
            <div style={{ display: 'inline-flex', alignItems: 'center', gap: 9, background: '#f1edf8', color: '#3d1e7b', fontWeight: 700, fontSize: 14, padding: '9px 15px', borderRadius: 999, marginBottom: 18 }}>
              <span style={{ width: 9, height: 9, borderRadius: '50%', background: '#dc8d32', display: 'inline-block' }} />{ADRESSE}
            </div>
            <p style={{ fontSize: 16, lineHeight: 1.65, color: '#4a4360', fontWeight: 500, margin: '0 0 22px' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(220px,1fr))', gap: 10, marginBottom: 26 }}>
              {FACTS.map((f) => (
                <div key={f} style={{ display: 'flex', alignItems: 'center', gap: 10, fontSize: 14.5, fontWeight: 600, color: '#1d1730' }}>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 12 }}>
              ...
              <Link href="/planning" style={{ textDecoration: 'none', background: 'none', color: '#3d1e7b', border: '1.5px solid #3d1e7b', fontWeight: 800, fontSize: 15, padding: '14px 24px', borderRadius: 999 }}>Voir le planning</Link>
            </div>
          </div>

          {/* Carte Google Maps (embed) */}
          <div style={{ borderRadius: 20, overflow: 'hidden', border: '1px solid #eee9f4', boxShadow: '0 20px 44px -26px rgba(23,18,43,.55)', minHeight: 360 }}>
            <iframe
              title="Plan d'accès"
      ...
      <section style={{ padding: '34px 26px 70px', maxWidth: 1240, margin: '0 auto' }}>
        ...
        <h2 style={{ fontFamily: "'Oswald',sans-serif", fontSize: 'clamp(24px,3vw,34px)', fontWeight: 700, textTransform: 'uppercase', color: '#1d1730', margin: '6px 0 24px' }}>En images</h2>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(260px,1fr))', gap: 16 }}>
          {gallery.map((im: any, i: number) => (
            <div key={i} style={{ position: 'relative', height: 230, borderRadius: 16, overflow: 'hidden', border: '1px solid #eee9f4', boxShadow: '0 14px 34px -26px rgba(23,18,43,.5)' }}>
              {im.ph ? (
                <div style={{ position: 'absolute', inset: 0, background: 'repeating-linear-gradient(45deg,#efeaf6,#efeaf6 11px,#e7e0f1 11px,#e7e0f1 22px)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: 'monospace', fontSize: 12, color: '#8a7fa6' }}>photo</div>
              ) : (
